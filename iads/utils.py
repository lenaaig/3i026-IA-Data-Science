...
class KernelPolynomial():
    def transform(self,x):
        ...
        for i in range(len(x)):
            if i+1 < len(x):
                vector.append(x[i]*x[i+1])
        return np.array(vector)
    
    
def split(p, labeledset):
    predict = ls.LabeledSet(labeledset.getInputDimension())
    train   = ls.LabeledSet(labeledset.getInputDimension())
    
    for i in range((int)(labeledset.size()*p)):
        train.addExample(labeledset.getX(i),labeledset.getY(i))
    for i in range((int)(labeledset.size()*p),labeledset.size()):
        predict.addExample(labeledset.getX(i),labeledset.getY(i))
